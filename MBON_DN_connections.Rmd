...
MBON_DS = cf_partner_summary(MBON$key, threshold = 5, partners = 'output')
MBON_DN = subset(MBON_DS, MBON_DS$type.post %in% DN$type)
nrow(MBON_DN)
# 55 direct MBONs to DN connections that are stronger than 5
length(unique(MBON_DN$type.pre))
# 11 MBONs that directly connect to DNs

MBON_DS = cf_partners(MBON$key, threshold = 5, partners = 'output')
MBON_DN = subset(MBON_DS, MBON_DS$type.post %in% DN$type)
```
